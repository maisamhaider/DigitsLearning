...
import com.example.digitslearning.adapters.HundredsNumbsAdapter;
import com.example.digitslearning.annotations.MAnnotation;
import com.example.digitslearning.models.HundredNumbersModel;

import java.util.ArrayList;

public class ExtraLanguagesNumActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_extra_languages_num);
        initArrays();

        RecyclerView extra_language_rv = findViewById(R.id.extra_language_rv);
        TextView textView6 = findViewById(R.id.textView6);
        ArrayList<HundredNumbersModel> list = new ArrayList<>();
        HundredsNumbsAdapter adapter;
        String s_extra_lang = getIntent().getStringExtra(MAnnotation.S_Extra_LANGUAGE_KEY);
        textView6.setText(s_extra_lang);
        if (s_extra_lang.matches(MAnnotation.ARABIC)) {
            list.add(new HundredNumbersModel(getOneTo100Arabic(), arabicWords, arabicEngPronounce));
            adapter = new HundredsNumbsAdapter(this, getOneTo100Arabic(), arabicWords, arabicEngPronounce);
        } else if (s_extra_lang.matches(MAnnotation.PERSIAN)) {
            list.add(new HundredNumbersModel(getOneTo100Arabic(), persianWords, persianEngPronounce));
            adapter = new HundredsNumbsAdapter(this, getOneTo100Arabic(), persianWords, persianEngPronounce);

        } else if (s_extra_lang.matches(MAnnotation.PAKHTU)) {
            list.add(new HundredNumbersModel(getOneTo100Arabic(), pakhtuWords, pakhtuEngPronounce));
            adapter = new HundredsNumbsAdapter(this, getOneTo100Arabic(), pakhtuWords, pakhtuEngPronounce);

        } else {
            list.add(new HundredNumbersModel(getOneTo100Arabic(), urduWords, urduEngPronounce));
            adapter = new HundredsNumbsAdapter(this, getOneTo100Arabic(), urduWords, urduEngPronounce);
        }

        LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
        extra_language_rv.setLayoutManager(layoutManager);
        extra_language_rv.setAdapter(adapter);
    ...
    public void initArrays() {
        ...
        urduEngPronounce = new String[]{
                "Sifr", "Aik", "Dow", "Teen", "Char", "Paanch", "chay", "Saat", "Oat", "Now", "Das",
                "Giarah", "Barah", "Terah", "Chaudah", "pandra", "Solah", "Sathrah", "Atharah", "Unnees",
                "Bees", "Ikkees", "Baais", "Taees", "Chaubees", "Pachees", "Chabbees", "Staees",
                "atayees", "Untees", "Tees", "Ikathees", "Bathees", "Thenthees", "Chauntees", "Pantees",
                "Chatees", "Santees", "Adthees", "Untaalees", "Chalees", "Iktalees", "Biyalees", "Tentalees",
                "Chavalees", "Pantalees", "Chiyalees", "Santalees", "Adtaalees", "Unchaas", "Pachaas",
                "Ikyavan", "Baavan", "Tarippan", "Chawwan", "Pachpan", "Chappan", "Staavan",
                "Athaavan", "Unsat", "Saat", "Iksat", "Baasat", "Traisat", "Chaunsat",
                "Painsat", "Chiyasat", "Sadsat", "Adsat", "Unhattar", "Sattar", "Ikhattar",
                "Behattar", "Tihattar", "Chauhattar", "Pachattar", "Chihattar", "Satattar",
                "Athhattar", "Unaasi", "Assi", "Ikyaasi", "Bayaasi", "Tiraasi", "Chaurassi",
                "Pachaasi", "Chiyaasi", "Sataasi", "Athaasi", "Navassi", "Navvay", "Ikyaanvey",
                "Baanvay", "Tiraanvay", "Chauraanvay", "Pachaanvay", "Chiyanvay", "Satanvay",
                "Athanvay", "Ninanvay", "Sow"};

        arabicEngPronounce = new String[]{
                "sayfir", "wahid", "isnan", "salasah", "arbaa", "khamsa", "sitha", "suba", "samanya",
                "thisa", "ashar", "ahada ashar", "isnan ashar", "salasah ashar", "arbaa ashar",
                "khamsa ashar", "sitta ashar", "suba ashar", "samanya ashar", "thisa ashar ",
                "ishrun", "wahid wa ishrun", "isnan wa ishrun", "salasah wa ishrun",
                ...
                "isnan wa salasun", "salasah wa salasun", "arbaa wa salasun",
                "khamsa wa salasun", "sitta wa salasun", "suba wa salasun",
                "samanya wa salasun", "thisa wa salasun", "arba-on", "wahid wa arba-on",
                "isnan wa arba-on", "salasah wa arba-on", "arbaa wa arba-on", "khamsa wa arba-on",
                "sitta wa arba-on", "suba wa arba-on", "samanya wa arba-on", "thisa wa arba-on",
                "khamson", "wahid wa khamson", "isnan wa khamson", "salasah wa khamson",
                ...
                "isnan wa sitton", "salasah wa sitton", "arbaa wa sitton", "hhamsa wa sitton",
                "sitta wa sitton", "suba wa sitton", "samanya wa sitton", "thisa wa sitton",
                "sub-on", "wahid wa sub-o", "isnan wa sub-o", "salasah wa sub-o",
                "arbaa wa sub-o", "khamsa wa sub-o", "ّitta wa sub-o", "suba wa sub-o ",
                "samanya wa sub-o", "thisa wa sub-o  ", "sama-nun", "ِwahid wa sama-nun ",
                " isnan wa sama-nun", "salasah wa sama-nun", "arbaa wa sama-nun", "hhamsa wa sama-nun",
                "sitta wa sama-nun", "suba wa sama-nun", "samanya wa sama-nun",
                "thisa wa sama-nun", "this-on", "wahid wa this-on", "isnan wa this-on ",
                "salasah wa this-on ", "arbaa wa this-on", "khamsa wa this-on", "sitta wa this-on",
                "suba wa this-on", "samanya wa this-on", "thisa wa this-on", "mi'a"};
        ...
        pakhtuEngPronounce = new String[]
                {"sifr", "yaw", "dwa", "dray", "saloor", "pindza", "špag", "wa", "ata", "naha", "lus",
                        "yow-lus", "dwo-lus", "dyaar-lus", "swaar-lus", "pinza-lus", "shipaa-ras",
                        "wa-las", "ata-lus", "no_nas", "shal", "yu-wisht", "du-wisht", "dray-wisht",
                        "saloor-wisht", "pindza-wisht", "špag-wisht", "wa-wisht", "ata-wisht",
                        "naha-wisht", "dersh", "yu-dersh", "dwa-dersh", "dray-dersh",
                        "saloor-dersh", "pindza-dersh", "špag-dersh", "wa-dersh", "ata-dersh",
                        "naha-dersh", "salwēk'ht", "yu-salwēk'ht", "dwa-salwēk'ht", "dray-salwēk'ht",
                        "saloor-salwēk'ht", "pindza-salwēk'ht", "špag-salwēk'ht", "wa-salwēk'ht",
                        "ata-salwēk'ht", "naha-salwēk'ht", "pendzos", "yu-pendzos", "dwa-pendzos",
                        "dray-pendzos", "saloor-pendzos", "pindza-pendzos", "špag-pendzos",
                        "wa-pendzos", "ata-pendzos", "naha-pendzos", "špeta", "yu-špeta", "dwa-špeta",
                        "dray-špeta", "saloor-špeta", "pindza-špeta", "špag-špeta",
                        "wa-špeta", "ata-špeta", "naha-špeta", "aûyâ", "yu-aûyâ", "dwa-aûyâ",
                        "dray-aûyâ", "saloor-aûyâ", "pindza-aûyâ", "špag-aûyâ",
                        "wa-aûyâ", "ata-aûyâ", "naha-aûyâ", "athyâ", "yu-athyâ", "dwa-athyâ",
                        "dray-athyâ", "saloor-athyâ", "pindza-athyâ", "špag-athyâ",
                        "wa-athyâ", "ata-athyâ", "naha-athyâ", "nawi", "yu-nawi", "dwa-nawi",
                        "dray-nawi", "saloor-nawi", "pindza-nawi", "špag-nawi", "wa-nawi",
                        "ata-nawi", "naha-nawi", "nawi", "sal"
                };}
    public ArrayList<String> getOneTo100Arabic() {
